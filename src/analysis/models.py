...
class DensePhaseDistributiveDeadenylation():
    """
    Distributive deadenylation assumes that enzyme falls off RNA strand after catalysis 
    and rebinds product to catalyze again, i.e. the enzyme is fully distributive. Also assumes that 
    enzyme cannot catalyze anything below TA2 in length, i.e. once only 1 A is left, the enzyme cannot
    ...
    """
    def __init__(self, fret_experiment):
        self.overall_model = "distributive-dense"
        self.get_dense_phase_values(fret_experiment,fret_experiment.partitioning_data)
        self.time = fret_experiment.time
        self.rna = fret_experiment.rna
        self.enzyme = fret_experiment.enzyme
        self.n = fret_experiment.n
    ...
    def get_dense_phase_values(self, fret_experiment, partitioning_data):
        ...
        for enz_conc in fret_experiment.enzyme:
            ...
            self.rna_dense.append(partitioning_data.loc[idx, 'Dense RNA'])
            self.enzyme_dense.append(partitioning_data.loc[idx,'Dense Enzyme'])
            self.DVF.append(partitioning_data.loc[idx,'Enzyme DVF'])
    ...
    def calculate_total_rna_concentrations(self):
        self.total_rna_concentrations_dense = {k:[] for k in self.concentrations.keys() if 'E' not in k}
        self.total_rna_concentrations = {k:[] for k in self.concentrations.keys() if 'E' not in k}
        for i, v in enumerate(self.enzyme_dense):
            for j in range(1, self.n+1):
                self.total_rna_concentrations_dense[f'TA{j}'].append([sum(x) for x in zip(self.concentrations[f'TA{j}'][i], self.concentrations[f'ETA{j}'][i])]) # TAi,T = [TAi] + [ETAi]
            self.total_rna_concentrations_dense['A1'].append(self.concentrations['A1'][i]) # TAi,T = [TAi] + [ETAi]
        for i, v in enumerate(self.enzyme_dense):
            for key in self.total_rna_concentrations_dense.keys(): # Scale dense phase concentrations back to total concentrations using DVF
                if key not in self.total_rna_concentrations:
                    self.total_rna_concentrations[key] = []
                self.total_rna_concentrations[key].append([x * self.DVF[i] for x in self.total_rna_concentrations_dense[key][i]])
    ...
    def simulate_kinetics(self, params):
        ...
        # for r, rna in enumerate(self.rna):
        for i, v in enumerate(self.enzyme):
            if len(self.rna_dense) == 1:
                rna = self.rna_dense[0]
            else:
                rna = self.rna_dense[i]
            if self.enzyme_dense[i] == 0: # No enzyme means nothing happens, all RNA is full length at all times
                self.concentrations[f'E*'].append([0 for x in range(len(self.time[i]))])
                self.concentrations[f'E'].append([0 for x in range(len(self.time[i]))])
                self.concentrations[f'A1'].append([0 for x in range(len(self.time[i]))])
                ...
                self.initial_concentration_guesses(self.enzyme_dense[i], rna, k1, km1, self.n)
                param_args = {'k1':k1, 'km1':km1, 'k2':k2, 'km2':km2, 'kcat':kcat, 'n':self.n}
                time_span = (np.min(self.time[i]),np.max(self.time[i]))
                initial_concs = self.C0
                rate_func = self.relaxation_matrix
...
class TwoPhaseSteadyStateDistributiveDeadenylation():
    ...
    """
    def __init__(self, fret_experiment, config_params):
    ...
    def get_phase_values(self, fret_experiment, partitioning_data):
        ...
        for enz_conc in fret_experiment.enzyme:
            ...
            if self.use_dvf == True:    
                ...
                self.rna_dilute.append(partitioning_data.loc[idx,'Dilute RNA'])
                self.rna_dense.append(partitioning_data.loc[idx,'Dense RNA'])
                self.enzyme_dilute.append(partitioning_data.loc[idx,'Dilute Enzyme'])
                self.enzyme_dense.append(partitioning_data.loc[idx,'Dense Enzyme'])

                self.rna_partition_coefficient.append(self.rna_dense[-1] / self.rna_dilute[-1])
                self.enz_partition_coefficient.append(self.enzyme_dense[-1] / self.enzyme_dilute[-1])
    ...
    def simulate_kinetics(self, params):
        ...
        for ri, rna_total in enumerate(self.rna):
            for ei, enzyme_total in enumerate(self.enzyme):
                ...
                rna = {}
                rna['Dilute']= rna_total / (DVF * (part_coeff - 1) + 1)
                ...
                if self.enzyme[ei] == 0: # No enzyme means nothing happens, all RNA is full length at all times
                    ...
                    solver_result = solve_ivp(self.rate_wrapper, time_span, initial_rna, t_eval=t_return, 
                                            method='Radau', first_step=1e-12, atol=1e-20, dense_output=True,
                                            args=(part_coeff, DVF, kcat, KM, self.n, enzyme))
                    self.extract_solved_concentrations_from_separate_rnas(t_return, self.time[ei], DVF)
    ...
    def sivp_wrapper(self, t_span, t_return, time_rnas, part_coeff, DVF, kcat, KM, n, enzyme):
        """        
        ...
        for di, dt in enumerate(t_return[1:]):
            t0 = t_return[di-1]
            t_span = (t0, dt)
            sol = solve_ivp(self.rate_wrapper, t_span, y, 
                        t_eval= [dt], method='BDF', first_step=1e-12, atol=1e-30, 
                        args=(part_coeff, DVF, kcat, KM, n, enzyme))
            y = sol.y[:, -1]
            y = [x if x > 0 else 0 for x in y] # Set any negative concentrations to zero
            results.append(y)
        return results

    def rate_wrapper(self, t_span, time_rnas, part_coeff, DVF, kcat, KM, n, all_enzyme):
        """        
        Given separate concentrations of RNA in the dilute and dense phases, calculate the total 
        concentration of each RNA species across both phases, then calculate the rates based on 
        the total concentrations. This is necessary because the kinetic model is based on total 
        ...
        sum_rnas = [x * DVF + y * dilVF for x,y in zip(rna_dense, rna_dilute)]
        ...
        rna_dilute = [Ct / (part_coeff * DVF + dilVF) for Ct in sum_rnas]
    ...
    def extract_solved_concentrations(self, solver_result, current_time, DVF):
        ...
        rna_dense = solver_result.y[0:self.n+1]
        rna_dilute = solver_result.y[self.n+1:]

        flipped_rna = []
        for ri, rna in enumerate([rna_dense, rna_dilute]):
            flipped_rna.append([]) # flipped_rna is list of two lists, one for dense and one for dilute phase RNA concentrations
            tmp = [[] for x in rna] # length of number of species
            for ti, time in enumerate(current_time): 
                idx = np.where(solver_result.t == time)[0][0]
                for ci, species_concentration in enumerate(rna):
        ...
        self.concentrations['A1'].append(sum_rnas[0])
        for x in range(1, self.n+1):
            self.concentrations[f"TA{x}"].append(sum_rnas[x])

        return flipped_rna

    def extract_solved_concentrations_from_separate_rnas(self, t_return, time_expt, DVF):
...
class TwoPhaseDeadenylation():
    """
    Two phase deadenylation is a work-in-progress. It allows for the modeling of RNA deadenylation
    in two distinct phases with either distributive or processive kinetics.

    No exchange between phases is currently implemented, making the model less flexible.

    Concentration and droplet volume information is necessary to implement the two-phase model effectively.
    """
    def __init__(self, fret_experiment, config_params):

        self.time = fret_experiment.time
        self.rna = fret_experiment.rna # total RNA
        self.enzyme = fret_experiment.enzyme # total enzyme
        self.enzyme_dilute = []
        self.rna_dilute = []
        self.enzyme_dense = []
        self.rna_dense = []
        ...
        self.rna_partition_coefficient = []
        self.totalvolume = []
        self.partition_concentrations(config_params, fret_experiment)
        self.n = fret_experiment.n

        fret_experiment.enzyme = self.enzyme_dilute
        fret_experiment.rna = self.rna_dilute
        if config_params["Modeling parameters"]["Submodels"]["Dilute model"] == "Distributive":
            self.kinetic_model_dilute = DistributiveDeadenylation(fret_experiment)
        elif config_params["Modeling parameters"]["Submodels"]["Dilute model"] == "Processive":
            self.kinetic_model_dilute = ProcessiveDeadenylation(fret_experiment)
        elif config_params["Modeling parameters"]["Submodels"]["Dilute model"] == "Steady state distributive":
            self.kinetic_model_dilute = SteadyStateDistributiveDeadenylation(fret_experiment)
        else:
            raise ValueError("Submodel Dilute model not recognized, must be either Distributive, Processive, or Steady state distributive.\n")
        
        fret_experiment.enzyme = self.enzyme_dense
        fret_experiment.rna = self.rna_dense
        if config_params["Modeling parameters"]["Submodels"]["Dense model"] == "Distributive":
            self.kinetic_model_dense = DistributiveDeadenylation(fret_experiment)
        elif config_params["Modeling parameters"]["Submodels"]["Dense model"] == "Processive":
            self.kinetic_model_dense = ProcessiveDeadenylation(fret_experiment)
        elif config_params["Modeling parameters"]["Submodels"]["Dense model"] == "Steady state distributive":
            self.kinetic_model_dense = SteadyStateDistributiveDeadenylation(fret_experiment)
        else:
            raise ValueError("Submodel Dense model not recognized, must be either Distributive or Processive.\n")
        
        fret_experiment.rna = self.rna
        fret_experiment.enzyme = self.enzyme   
        self.species_list()


    def partition_concentrations(self, config_params, fret_experiment):
        # Read concentration data from a CSV file and partition it into kinetic_model_dilute and kinetic_model_dense
        with open(config_params["Partitioning"]["Data file"], 'r', encoding='utf-8-sig') as dataload:
            data = dataload.read()
            data = data.split('\n')
            values = []
        ...
        part_data = {names[i]:values[i] for i in range(len(names))}

        def find_index(concentrations, target_rna, target_enzyme):
            for i, (rna, enzyme) in enumerate(zip(concentrations['Total RNA'], concentrations['Total Enzyme'])):
                if rna == target_rna and enzyme == target_enzyme:
                    return i
            return -1
        
        for e in fret_experiment.enzyme:
            index = find_index(part_data, 1e-07, e)
            try:
                if index != -1:
                    self.enzyme_dilute.append(part_data["Dilute Enzyme"][index])
                    self.rna_dilute.append(part_data["Dilute RNA"][index])
                    self.enzyme_dense.append(part_data["Dense Enzyme"][index])
                    self.rna_dense.append(part_data["Dense RNA"][index])
                    self.DVF_enzyme.append(part_data["Enzyme DVF"][index])
                    self.DVF_rna.append(part_data["RNA DVF"][index])
                    self.totalvolume.append(part_data["Total Volume"][index])
                    self.rna_partition_coefficient.append(part_data["Dense RNA"][index] / part_data["Dilute RNA"][index])
            except:
                print("\nPartitioning data not found for enzyme: ", e)
                exit()

    def species_list(self):
        species_dilute = self.kinetic_model_dilute.species
        species_dense = self.kinetic_model_dense.species
        self.species = [str(x) for x in np.unique(species_dilute + species_dense)]

    def setup_concentrations(self):
        self.concentrations = {}
        self.concentrations_dense = {}
        self.concentrations_dilute = {}
        for spec in self.species:
            self.concentrations[spec] = []
            self.concentrations_dense[spec] = []
            self.concentrations_dilute[spec] = []

    def extract_solved_concentrations(self):  
        for i, v in enumerate(self.species):
            m1 = 0
            m2 = 0
            if v in self.kinetic_model_dilute.concentrations.keys():
                m1 = 1
            if v in self.kinetic_model_dense.concentrations.keys():
                m2 = 1
            if m1 == 1 and m2 == 1:
                for j in range(len(self.kinetic_model_dilute.concentrations[v])): # each species has an array for each enzyme concentration (j)
                    DVF = self.DVF_enzyme[j]
                    diluteVol = self.totalvolume[j] * (1 - self.DVF_enzyme[j])
                    denseVol = self.totalvolume[j] * self.DVF_enzyme[j] 
                    self.concentrations[v].append([(x * (1-DVF))+(y * DVF) for x,y in zip(self.kinetic_model_dilute.concentrations[v][j],self.kinetic_model_dense.concentrations[v][j])])
                    self.concentrations_dense[v].append([x * (DVF) for x in self.kinetic_model_dense.concentrations[v][j]])
                    self.concentrations_dilute[v].append([x * (1-DVF) for x in self.kinetic_model_dilute.concentrations[v][j]])
            elif m1 == 1 and m2 == 0:
                self.concentrations[v] = self.kinetic_model_dilute.concentrations[v]
            elif m1 == 0 and m2 == 1:
                self.concentrations[v] = self.kinetic_model_dense.concentrations[v]
    ...
    def simulate_kinetics(self, params):
        ...
        for phase in ["Dilute", "Dense"]:
            phase_params = [k.replace(f"_{phase}","") for k in params if phase in k]
            for param in phase_params:
                input_params[param] = params[f'{param}_{phase}']
            if phase == "Dilute":
                self.kinetic_model_dilute.simulate_kinetics(input_params)
            elif phase == "Dense":
                self.kinetic_model_dense.simulate_kinetics(input_params)

        self.extract_solved_concentrations()
...
class DuplexHybridization:
    ...
    def get_total_rna_concentrations(self, prior_to_hybridization_concentrations):
        if any(self.fit_model == x for x in ['Steady state distributive', '2phase SS distributive']):
...
def generate_model_objects(fret_experiment, fit_model, config_params):
    ...
    if fit_model == 'Distributive-dense':
        kinetic_model = DensePhaseDistributiveDeadenylation(fret_experiment)
    if fit_model == 'Processive':
        kinetic_model = ProcessiveDeadenylation(fret_experiment)
    if fit_model == '2phase':
        kinetic_model = TwoPhaseDeadenylation(fret_experiment, config_params)
    if fit_model == '2phase SS distributive':
        kinetic_model = TwoPhaseSteadyStateDistributiveDeadenylation(fret_experiment, config_params)
